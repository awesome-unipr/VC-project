...
class TkinterGui:
    def __init__(self):
        ...
        self.driver_text = self.keycv.create_text(
            self.keycv.winfo_width() / 2,
            self.keycv.winfo_height() / 4,
            text="",
            fill="#08c044",
            font=("Small Fonts", 14),
        )
        self.key_icon = self.key_icon.resize(
            (self.keycv.winfo_width(), self.keycv.winfo_height())
        )
        self.tk_key = ImageTk.PhotoImage(self.key_icon)
        self.keycv.create_image(0, 0, image=self.tk_key, anchor=tk.NW)

        self.lock_icon = self.lock_icon.resize(
            (self.lockcv.winfo_width(), self.lockcv.winfo_height())
        )
        self.tk_lock = ImageTk.PhotoImage(self.lock_icon)
        self.lockcv.create_image(0, 0, image=self.tk_lock, anchor=tk.NW)

        self.sec_icon = self.sec_icon.resize(
            (self.seccv.winfo_width(), self.seccv.winfo_height())
        )
        self.tk_sec = ImageTk.PhotoImage(self.sec_icon)
        self.seccv.create_image(0, 0, image=self.tk_sec, anchor=tk.NW)

        self.abs_icon = self.abs_icon.resize(
            (self.abscv.winfo_width(), self.abscv.winfo_height())
        )
        ...
        self.radio_frame = Frame(
            self.right_frame, highlightbackground="#36454F", highlightthickness=6
        )
        ...
        self.prev_st = Button(
            self.radio_frame, text="<", bg="#36454F", fg="white", font=("System", 20)
        )
        self.next_st = Button(
            self.radio_frame, text=">", bg="#36454F", fg="white", font=("System", 20)
        )
        ...
        for i in range(10):
            self.state_frame[i] = Frame(
                self.dms_frame,
                bg=self.dms_bg,
                highlightbackground="#36454F",
                highlightthickness=2,
            )
            self.state_frame[i].place(
                relx=0, rely=0.1 * i, relwidth=1, relheight=0.1, anchor=tk.NW
            )
            ...
            self.state[i].place(
                relx=0.3, rely=0, relwidth=0.5, relheight=1, anchor=tk.NW
            )
        ...
        self.weather_frame = Frame(
            self.right_frame,
            background=self.weather_bg,
            highlightbackground="#36454F",
            highlightthickness=6,
        )
        self.weather_frame.place(
            relx=0.55, rely=0.2, relwidth=0.45, relheight=0.8, anchor=tk.NW
        )

        self.current_weather_frame = Frame(
            self.weather_frame,
            bg=self.weather_bg,
            highlightbackground="#36454F",
        ...
        self.current_weather_frame.place(
            relx=0, rely=0, relwidth=1, relheight=0.3, anchor=tk.NW
        )

        self.current_city = Label(
        ...
        self.cw_cv = tk.Canvas(
            self.current_weather_frame, background=self.weather_bg, highlightthickness=0
        )
        ...
        self.current_temperature = Label(
            self.current_weather_frame,
            text="----",
            bg=self.weather_bg,
            fg="white",
        ...
        self.current_city.place(
            relx=0, rely=0, relwidth=0.5, relheight=0.5, anchor=tk.NW
        )
        self.current_time.place(
            relx=0.5, rely=0, relwidth=0.5, relheight=0.5, anchor=tk.NW
        )
        self.cw_cv.place(
            relx=0.05, rely=0.5, relwidth=0.25, relheight=0.5, anchor=tk.NW
        )
        self.current_weather.place(
            relx=0.3, rely=0.5, relwidth=0.5, relheight=0.5, anchor=tk.NW
        )
        self.current_temperature.place(
            relx=0.8, rely=0.5, relwidth=0.2, relheight=0.5, anchor=tk.NW
        )
        ...
        self.cloud_icon = self.cloud_icon.resize(
            (self.cw_cv.winfo_width(), self.cw_cv.winfo_height())
        )
        self.tk_cloud = ImageTk.PhotoImage(self.cloud_icon)
        self.sun_icon = self.sun_icon.resize(
            (self.cw_cv.winfo_width(), self.cw_cv.winfo_height())
        )
        self.tk_sun = ImageTk.PhotoImage(self.sun_icon)
        self.pcloud_icon = self.pcloud_icon.resize(
            (self.cw_cv.winfo_width(), self.cw_cv.winfo_height())
        )
        self.tk_pcloud = ImageTk.PhotoImage(self.pcloud_icon)
        self.rain_icon = self.rain_icon.resize(
            (self.cw_cv.winfo_width(), self.cw_cv.winfo_height())
        )
        self.tk_rain = ImageTk.PhotoImage(self.rain_icon)
        self.snow_icon = self.snow_icon.resize(
            (self.cw_cv.winfo_width(), self.cw_cv.winfo_height())
        )
        self.tk_snow = ImageTk.PhotoImage(self.snow_icon)
        self.mist_icon = self.mist_icon.resize(
            (self.cw_cv.winfo_width(), self.cw_cv.winfo_height())
        )
        self.tk_mist = ImageTk.PhotoImage(self.mist_icon)
        self.storm_icon = self.storm_icon.resize(
            (self.cw_cv.winfo_width(), self.cw_cv.winfo_height())
        )
        self.tk_storm = ImageTk.PhotoImage(self.storm_icon)
        self.defweather_icon = self.defweather_icon.resize(
            (self.cw_cv.winfo_width(), self.cw_cv.winfo_height())
        )
        ...
        for i in range(7):
            self.forecast_frame[i] = Frame(
                self.weather_frame,
                bg=self.weather_bg,
                highlightbackground="#36454F",
                highlightthickness=2,
            )
            self.forecast_frame[i].place(
                relx=0, rely=0.3 + 0.1 * i, relwidth=1, relheight=0.1, anchor=tk.NW
            )

            self.time[i] = Label(
                self.forecast_frame[i],
                text="00:00",
                bg=self.weather_bg,
                fg="white",
                font=("Small Fonts", 14),
            )
            self.fw_cv[i] = tk.Canvas(
                self.forecast_frame[i], background=self.weather_bg, highlightthickness=0
            )
            self.weather[i] = Label(
                self.forecast_frame[i],
                text="--------",
                bg=self.weather_bg,
                fg="white",
            ...
            self.fw_cv[i].place(
                relx=0.25, rely=0, relwidth=0.15, relheight=1, anchor=tk.NW
            )
            self.weather[i].place(
                relx=0.4, rely=0, relwidth=0.4, relheight=1, anchor=tk.NW
            )
            self.temperature[i].place(
                relx=0.8, rely=0, relwidth=0.2, relheight=1, anchor=tk.NW
            )
        ...
        self.settings_button.place(
            relx=0.98, rely=0.02, width=50, height=50, anchor=tk.NE
        )
        self.settings_button.config(command=self.open_settings)

        self.settings_frame = Frame(
            self.window, highlightbackground="black", highlightthickness=2
        )
        ...
        self.settings_button.place(
            relx=0.04, rely=0.02, width=50, height=50, anchor=tk.NW
        )
        self.settings_button.config(command=self.close_settings)

        self.settings_title = Label(
            self.settings_frame, text="Control Settings", fg="black", font=("Arial", 20)
        )
        self.settings_title.place(relx=0.90, rely=0.03, anchor=tk.NE)

        self.login_driver = Button(
            self.settings_frame,
            text="Access Vehicle",
            highlightbackground="black",
            highlightthickness=2,
        ...
        self.login_driver.place(
            relx=0.04, rely=0.2, relwidth=0.92, height=50, anchor=tk.NW
        )

        self.logout_driver = Button(
            self.settings_frame,
            text="Leave Vehicle",
            highlightbackground="black",
            highlightthickness=2,
        ...
        self.logout_driver.place(
            relx=0.04, rely=0.3, relwidth=0.92, height=50, anchor=tk.NW
        )

        self.unrecognized_driver = Button(
            self.settings_frame,
            text="Urecognized Access",
            highlightbackground="black",
            highlightthickness=2,
            bg="#36454F",
            fg="white",
        )
        self.unrecognized_driver.place(
            relx=0.04, rely=0.4, relwidth=0.92, height=50, anchor=tk.NW
        )

        self.random_weather = Button(
            self.settings_frame,
            text="Set Random Weather",
            highlightbackground="black",
            highlightthickness=2,
            bg="#36454F",
            fg="white",
        )
        self.random_weather.place(
            relx=0.04, rely=0.5, relwidth=0.92, height=50, anchor=tk.NW
        )
...
class VehicleStatus:
    def __init__(self, gui, cobu_port, skr_port):
        ...
        self._gui.window.after(
            self.upd_interval, lambda: asyncio.run(self.update_cycle())
        )
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    url="http://localhost:" + str(self.rks_port) + "/driver"
                ) as response:
    ...
    def update_gui(self):
        ...
        try:
            ...
            self._gui.current_time.config(
                text=str(now.time().hour) + ":" + str(now.time().minute)
            )
            self._gui.current_weather.config(text=self.current_weather["weather"])
            self._gui.current_temperature.config(
            ...
            for i, w in enumerate(self.forecast_weather):
                self._gui.time[i].config(
                    text=str(self.forecast_weather[w]["time"]) + ":00"
                )
                self._gui.weather[i].config(text=self.forecast_weather[w]["weather"])
                self._gui.temperature[i].config(
                    text=str(self.forecast_weather[w]["temperature"]) + " °F"
                )
                self.display_w_icon(
        ...
        try:
            async with aiohttp.ClientSession() as session:
                ...
                async with session.post(
                    url="http://localhost:" + str(self._port) + "/radio-change/",
                    data=payload,
                ) as response:
...
class WeatherInformation:
    ...
    async def set_weather(self):
        ...
        for i in range(0, 7):
            item = {
                i: {
                    "time": str(forecast_times[i]),
                    "temperature": str(forecast_temperatures[i]),
                    "kind": forecast_weathers[i],
                }
            }
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                url="http://localhost:" + str(self._port) + "/weathercurrent",
                data=payload_current,
            ) as response:
                print(response)

            async with session.post(
                "http://localhost:" + str(self._port) + "/weatherforecast",
...
class RemoteKeylessSystem:
    def __init__(self, gui, port):
        ...
        self._gui.logout_driver.config(
            command=lambda: asyncio.run(self.logout_driver())
        )
        self._gui.unrecognized_driver.config(
            command=lambda: asyncio.run(self.unrecognized_driver())
        )
    ...
    async def login_driver(self):
        try:
            async with aiohttp.ClientSession() as session:
                ...
                async with session.post(
                    url="http://localhost:" + str(self.port) + "/keyless", json=payload
                ) as response:
                    await response.text()
    ...
    async def logout_driver(self):
        try:
            async with aiohttp.ClientSession() as session:
                ...
                async with session.post(
                    url="http://localhost:" + str(self.port) + "/logout", json=payload
                ) as response:
                    await response.text()
